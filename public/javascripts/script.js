...
sync.playlist = (function() {
    ...
    self.addVideo = function(video) {
        ...
        sync.chat.append("<strong>" + video.via + "</strong> iniciou: <em>" + video.title + "</em>", 'color:green');
...
(function () {
    ...
    socket.emit("join", user, roomName);
    socket.on("ackJoin", function (data) {
        sync.users.uid = data.uid;
        sync.chat.append( "<strong>Entrou no Feat " + roomName + ".</strong>" +
                          "<br>Usuário '" + data.username + "'. <br>"
